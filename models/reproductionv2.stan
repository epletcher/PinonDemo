...
model {
    ...
    cp[t,j] ~ neg_binomial_2(exp(alpha2[t] + beta2[t]*Sz[t,j,2]),phi2); // unlogged size, under avg growth conditions

    cp[t,j] ~ neg_binomial_2(exp(alpha3[t] + beta3[t]*Sz[t,j,3]),phi3); // unlogged size, under high growth conditions
  ...
  // 
  phi1 ~ inv_gamma(0.4, 0.3); // inv_gamma(0.4, 0.3) or cauchy(0,5), but this allows for negatives
